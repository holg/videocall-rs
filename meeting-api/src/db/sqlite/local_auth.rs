//! Local user authentication queries (SQLite).
...
pub async fn find_by_invite_token(
    ...
    sqlx::query_as::<_, LocalUserRow>(
        ...
        SELECT id, email, name, password_hash, invite_token, invite_expires_at,
               activated_at, created_at, last_login
        FROM local_users
        WHERE invite_token = ?1 AND activated_at IS NULL AND invite_expires_at > datetime('now')
        "#,
    )
    .bind(invite_token)
    .fetch_optional(pool)
...
pub async fn activate_user(
    ...
    sqlx::query_as::<_, LocalUserRow>(
        r#"
        UPDATE local_users
        SET password_hash = ?2, activated_at = datetime('now')
...
pub async fn update_last_login(pool: &DbPool, id: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE local_users SET last_login = datetime('now') WHERE id = ?1")
        .bind(id)
        .execute(pool)
        .await?;
    Ok(())
